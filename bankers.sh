...
# available resources
echo "Enter available resources (separated ny spaces):"
read -ra available
...
while $processFlag; do
    # and immediately set it as false so if there are no remaining possible processes to execute, exit the loop
    processFlag=false
